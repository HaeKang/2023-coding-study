# reference: https://resilient-923.tistory.com/357
            ...
            if next_y >= 0 and next_y < N and next_x >= 0 and next_x < N and visited[next_y][next_x] == 0:
                if area[next_y][next_x] <= size:
                    ...
                    if area[next_y][next_x] < size and area[next_y][next_x] != 0:
                        cand.append((visited[next_y][next_x]-1, next_y, next_x))
    return sorted(cand, key=lambda x:(x[0], x[1], x[2])) # x[0], x[1], x[2] 순으로 정렬: 거리 순으로 정렬한다는 뜻 
...
while True:
    ...
    cnt += 1
    if cnt == size: # 먹은 숫자가 현재 크기일 때 크기 업
        size += 1
        cnt = 0
